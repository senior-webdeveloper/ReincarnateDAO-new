...
import { bnb, hxl_usdt, hxl_busd } from "./AllBonds";
...
export async function calcRunway(circulatingSupply: number, { networkID, provider }: IBaseAsyncThunk) {
  const reserves = [
    ...
    bnb.networkAddrs[networkID].reserveAddress,
  ];
  const lps = [hxl_usdt.networkAddrs[networkID].reserveAddress, hxl_busd.networkAddrs[networkID].reserveAddress];
  const wftmBondContract = new ethers.Contract(bnb.networkAddrs[networkID].bondAddress, [assetPrice], provider);
  ...
  for (const reserve of reserves) {
    ...
    const price =
      reserve == bnb.networkAddrs[networkID].bondAddress ? (await wftmBondContract.assetPrice()) / 10 ** 8 : 1;

...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bond, value, provider, networkID }: ICalcBondDetailsAsyncThunk, { dispatch }): Promise<IBondDetails> => {
    ...
    let isSoldOut = false;
    console.log("GDfsf", bond.name, terms );
    ...
    try {
      ...
      console.log("bondPrice_debug: ", bond.name,bondPrice.toString());
    ...
    if (Number(value) === 0) {
      ...
      console.log("AmountInWei_",bond.name, amountInWei, bondQuote);
...
export const bondAsset = createAsyncThunk(
  ...
  async ({ value, address, bond, networkID, provider, slippage }: IBondAssetAsyncThunk, { dispatch }) => {
    ...
    try {
      console.log("bonddeposit_debug",bond,valueInWei, maxPremium, depositorAddress)
      bondTx = await bondContract.deposit(valueInWei, maxPremium, depositorAddress);
      console.log("bonddeposit_debug_result",bondTx)
      dispatch(
